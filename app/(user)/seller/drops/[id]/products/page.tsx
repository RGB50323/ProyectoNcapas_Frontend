...
export default function SellerDropProductsPage() {
  ...
  const refresh = async () => {
    if (!session) return
    const [drops, dps, mine] = await Promise.all([
      admin.listMyDrops(session),
  ...
  const available = myProducts.filter((p) => !attachedIds.has(p.id))
  const productById = useMemo(() => new Map(myProducts.map((p) => [p.id, p])), [myProducts])
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 8 }}>
        <Link className="mono accent" style={{ fontSize: 11 }} href="/seller/drops">← VOLVER A DROPS</Link>
      </div>
