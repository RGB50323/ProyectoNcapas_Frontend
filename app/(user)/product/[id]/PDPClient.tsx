        ...
        <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: 16 }}>
            ...
            <div style={{ position: 'relative', aspectRatio: '4/5', overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--card)' }}>
                ...
                <div style={{ position: 'absolute', top: 20, left: 20, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {badges.map((badge) => (
                        <span key={badge} className={'badge ' + (badge.toLowerCase().includes('verificado') ? 'verified' : badge.toLowerCase().includes('limitado') ? 'limited' : '')}>
              {badge.toUpperCase()}
            </span>
                    ))}
                </div>
...
export default function PDPClient({ product, reviews, similar }: { product: Product; reviews: Review[]; similar: Product[] }) {
    ...
    // Cargar alerta existente del buyer
    useEffect(() => {
        if (!session || session.role !== 'BUYER') return
        getMyStockAlerts(session).then((alerts) => {
    ...
    // Toggle alerta de stock
    const toggleAlert = async () => {
        if (!session) { router.push('/login'); return }
        setAlertBusy(true)
        try {
            if (alertId) {
                await deleteStockAlert(alertId, session)
                setAlertId(null)
                show('Alerta de stock cancelada.', 'success')
            } else {
                await createStockAlert(product.id, session)
                const alerts = await getMyStockAlerts(session)
                const newAlert = alerts.find((a) => a.productId === product.id)
                setAlertId(newAlert?.id ?? 'set')
                show('Te avisaremos cuando haya stock disponible.', 'success')
            }
        } catch {
            show('No se pudo gestionar la alerta.', 'error')
        } finally {
            setAlertBusy(false)
        }
    }
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 32 }}>
                        {product.soldOut ? (
                            <button
                                ...
                                style={{
                                    flex: 1,
                                    background: alertId ? 'var(--card)' : 'var(--text)',
                                    color: alertId ? 'var(--accent-2)' : 'var(--bg-0)',
                                    border: alertId ? '1px solid var(--accent-2)' : 'none',
                                }}
                                disabled={alertBusy}
                                onClick={toggleAlert}
                            >
                            ...
                            // Producto disponible → botón normal
                            <button
                                className="btn btn-lg"
                                style={{ flex: 1 }}
                                disabled={adding}
                                onClick={addToBag}
                            >
                                {adding ? 'Agregando…' : `Añadir a la bolsa · $${product.price}`}
                            </button>
                        )}
                    ...
                    <div style={{ marginTop: 32, borderTop: '1px solid var(--border)' }}>
                        {details.map(([t, b], i) => (
                            <div key={t} style={{ borderBottom: '1px solid var(--border)' }}>
                                <button onClick={() => toggleDetail(i)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0', width: '100%', background: 'none', border: 'none', color: 'var(--text)', textAlign: 'left', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                    {reviews.map((r: any) => (
                        <div key={r.id} className="card" style={{ padding: 24 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                                <div className="display" style={{ fontSize: 14 }}>{r.userFirstName} {r.userLastName}</div>
                                <div className="mono mute">
                                    {new Date(r.createdAt).toLocaleDateString('es-SV', { day: 'numeric', month: 'short', year: 'numeric' })}
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: 2, color: 'var(--accent-2)', marginBottom: 12 }}>
                                {[1, 2, 3, 4, 5].map((j) => <Icon.Star key={j} filled={j <= r.rating} />)}
                            </div>
                            <p style={{ color: 'var(--text-dim)', fontSize: 13, lineHeight: 1.7, margin: 0 }}>&ldquo;{r.body}&rdquo;</p>
